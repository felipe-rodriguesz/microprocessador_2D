program simulacao_termica_cn
    ...
    do n = 1, Nt
        ...
        if ( mod(tempo + dt, periodo) < (periodo / 2.0d0) ) then
            S_next = 1.0d0
        else
            S_next = 0.0d0
        end if
        ...
        do iter = 1, max_iter
            ...
            do j = 2, Ny-1
                do i = 2, Nx-1
                    ...
                    ! Equação Iterativa do Crank-Nicolson usando o RHS fixo
                    T_new(i, j) = (RHS(i,j) &
                                + Fx * (T_new(i+1, j) + T_new(i-1, j)) &
                                + Fy * (T_new(i, j+1) + T_new(i, j-1))) / den

                    ! Norma relativa (Acumula o quadrado do erro)
                    soma_erro = soma_erro + ((T_new(i,j) - T_old_iter) / T_new(i,j))**2

                    ! Calcula o resíduo
                    res_ij = T_new(i,j) - (RHS(i,j) &
                           + Fx*(T_new(i+1,j) + T_new(i-1,j)) &
                           + Fy*(T_new(i,j+1) + T_new(i,j-1))) / den
                    if (abs(res_ij) > residuo_max) residuo_max = abs(res_ij)
                end do
            end do
...
end program simulacao_termica_cn